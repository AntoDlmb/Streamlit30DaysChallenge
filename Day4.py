...
#construction des données de benchmark
df_agg_diff = df_agg.copy()
metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(months =12)
#on filtre sur les valeurs numériques
df_numeric = df_agg_diff.select_dtypes(include=[np.number])
median_agg = df_numeric[df_agg_diff['Video publish time'] >= metric_date_12mo].median()
...
if add_sidebar == 'Aggregate Metrics':
    ...
    count = 0
    for i in metric_medians6mo[1:].index:
        with columns[count]:
    ...
    df_to_pct = {}
    for c in df_agg_diff_final.columns:
        if df_agg_diff_final[c].dtypes.kind in 'if':
            df_to_pct[c] = '{:.1%}'.format
